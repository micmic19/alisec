<?
//<title>Alice yandex</title>
//#ALICE В эту переменную можно писать отладочную информацию
//$arRunErrors[] = $strOfferYandex;
//#ALICE
...
else
{
	...
	//#ALICE
	$ar_iblock["SERVER_NAME"] = COption::GetOptionString("main", "server_name", "");
	$res = CIBlockElement::getlist(array(), array("IBLOCK_ID"=>5, 'CODE' => $SETUP_SERVER_NAME), false, false, Array("ID", "PROPERTY_YANDEX_DELIVERY_AMOUNT", "PROPERTY_YANDEX_DELIVERY_TIME"));
	while($ob = $res->GetNextElement()){ 
		$ar_fields = $ob->GetFields();
		$YANDEX_DELIVERY_AMOUNT = $ar_fields["PROPERTY_YANDEX_DELIVERY_AMOUNT_VALUE"];
		$YANDEX_DELIVERY_TIME = $ar_fields["PROPERTY_YANDEX_DELIVERY_TIME_VALUE"];		
	}
	//#ALICE
...
if (empty($arRunErrors))
{
	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		//$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL");
 		$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL");
		//#ALICE
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
	...
	elseif ('P' == $arCatalog['CATALOG_TYPE'] || 'X' == $arCatalog['CATALOG_TYPE'])
	{
		...
		//#ALICE
 		$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL");	
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
					...
					if ($minPrice <= 0)
						continue;
					//#ALICE
					$ar_product = CCatalogProduct::GetByID($arOfferItem['ID']);
					if ($ar_product['QUANTITY'] <= 0)
						continue;
					//#ALICE
					if (strlen($arOfferItem['DETAIL_PAGE_URL']) <= 0)
						$arOfferItem['DETAIL_PAGE_URL'] = '/';
					...
					//#ALICE
					/*$strOfferYandex .= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?></url>\n";*/
					$strOfferYandex.= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?>&amp;cityid=".$SETUP_SERVER_NAME."&amp;sku=".$arOfferItem["ID"]."</url>\n";//#ALICE					
					//#ALICE
					$strOfferYandex .= "<price>".$minPrice."</price>\n";
					$strOfferYandex .= "<currencyId>".$minPriceCurrency."</currencyId>\n";
					...
					//#ALICE
					$ar_user_fields = GetSectionUserFields($IBLOCK_ID, $arItem["ID"]);//#ALICE
					if (!empty($ar_user_fields["UF_MARKET_CATEGORY"]))
						$strOfferYandex.= "<market_category>".yandex_text2xml($ar_user_fields["UF_MARKET_CATEGORY"])."</market_category>\n";
					$strOfferYandex .=  "<store>false</store>\n";
					$strOfferYandex .= "<pickup>true</pickup>\n";
					$strOfferYandex .= "<delivery>true</delivery>\n";
					$strOfferYandex .= "<country_of_origin>" . yandex_text2xml($arItem['PROPERTIES']['60']['VALUE']) . "</country_of_origin>\n";
					...
					//AddMessage2Log(print_r($arItem,true), "sale");
					//#ALICE
					$y = 0;
					foreach ($arYandexFields as $key)
					{
						...
						case 'description':
							...
							{
								//#ALICE
								//$strOfferYandex .= $arItem['YANDEX_TEXT'];
								$strOfferYandex .= yandex_text2xml($arItem['~DETAIL_TEXT']);
								//#ALICE
							}
							else
							{
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				//#ALICE
				$arItem["~DETAIL_PAGE_URL"] = str_replace('b2b/', '', $arItem["~DETAIL_PAGE_URL"]); 
				...
				$strOfferYandex.= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]).(strstr($arItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=<?echo \$strReferer1; ?>&amp;r2=<?echo \$strReferer2; ?>&amp;cityid=".$SETUP_SERVER_NAME."</url>\n";
				//#ALICE
	...
	fwrite($fp, "</categories>\n");
	//#ALICE
	if ($YANDEX_DELIVERY_AMOUNT > 0)
		@fwrite($fp, "<delivery-options>\n  <option cost=\"".$YANDEX_DELIVERY_AMOUNT."\" days=\"".$YANDEX_DELIVERY_TIME."\"/>\n</delivery-options>\n");
	//#ALICE
	fwrite($fp, "<offers>\n");
	fwrite($fp, $strTmpOff);
	fwrite($fp, "</offers>\n");
