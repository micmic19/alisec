...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem["PROPERTIES"]["MATERIAL"]["VALUE"] == "Шерсть" || $arItem["PROPERTY_11"] == "Шерсть") {
			$arItem['LABEL_VALUE'] = "Шерсть";
			$arItem['LABEL'] = true;
		}
		//ALICE Акция
		foreach ($arItem["OFFERS"] as $keyOffer => $arItemOffer)
			if ($arItemOffer["PROPERTIES"]["IN_ACTION"]["VALUE"] == 1){
			$arItem['LABEL_VALUE'] = "Акция";
			$arItem['LABEL'] = true;
		}
		//ALICE Акция
		//ALICE бугры
...
//			$arItem['LABEL_VALUE'] = "на остаток в п. Бугры";
//			$arItem['LABEL'] = true;
//			$arItem['LABEL_SKU'] = true;
//			}	
//		}
		//ALICE бугры
        foreach ($arItem['PRICES'] as $keyPrice => &$arPrice){
            ...
            {
                $arPrice['DISCOUNT_DIFF_PERCENT'] = 0;
                $arPrice['DISCOUNT_DIFF'] = 0;
                $arPrice['PRINT_DISCOUNT_DIFF'] = FormatCurrency(0, $arPrice['CURRENCY']);
            }
            else
            {
				$arPrice['DISCOUNT_VALUE'] = round($arPrice['DISCOUNT_VALUE']);
				$arPrice['PRINT_DISCOUNT_VALUE'] = FormatCurrency($arPrice['DISCOUNT_VALUE'], $arPrice['CURRENCY']);
                $arPrice['DISCOUNT_DIFF'] = $arPrice['VALUE'] - $arPrice['DISCOUNT_VALUE'];
                $arPrice['PRINT_DISCOUNT_DIFF'] = FormatCurrency($arPrice['DISCOUNT_DIFF'], $arPrice['CURRENCY']);
            }
        }
		$arItem["MIN_PRICE"]["DISCOUNT_VALUE"] = round($arItem["MIN_PRICE"]["DISCOUNT_VALUE"]);
		$arItem["MIN_PRICE"]['PRINT_DISCOUNT_VALUE'] = FormatCurrency($arItem["MIN_PRICE"]["DISCOUNT_VALUE"], $arItem["MIN_PRICE"]['CURRENCY']);
		$arItem["MIN_PRICE"]['DISCOUNT_DIFF'] = $arItem["MIN_PRICE"]['VALUE'] - $arItem["MIN_PRICE"]["DISCOUNT_VALUE"];
		$arItem["MIN_PRICE"]['PRINT_DISCOUNT_DIFF'] = FormatCurrency($arItem["MIN_PRICE"]['DISCOUNT_DIFF'], $arItem["MIN_PRICE"]['CURRENCY']);
		//ItemName = UF_GROUP_NAME + ItemName
		$arFilterSection = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'GLOBAL_ACTIVE'=>'Y', 'ID'=>$arItem['IBLOCK_SECTION_ID']);
		...
		if($ar_result = $db_list->GetNext()){
			if (empty($ar_result['IBLOCK_SECTION_ID']) || !empty($ar_result['UF_GROUP_NAME']))
				if (empty($ar_result['UF_GROUP_NAME']))
					$arItem['NAME'] = $ar_result['NAME']." ".$arItem['NAME'];
				else
				    $arItem['NAME'] = $ar_result['UF_GROUP_NAME']." ".$arItem['NAME'];
			else{	
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$restestofferprop = testofferprop($arOffer["PROPERTIES"]["LENGTH"]["VALUE"], $arOffer["PROPERTIES"]["WIDTH"]["VALUE"], $FILTER_VALUE["OFFERS"]["=PROPERTY_47"], $FILTER_VALUE["OFFERS"]["=PROPERTY_48"]);
					if (!($restestofferprop && $arOffer["CAN_BUY"]==true)) 
						continue;
					$arOfferSizes[$arOffer["PROPERTIES"]["SIZES"]["VALUE_ENUM_ID"]] = array("ID"=>$arOffer["PROPERTIES"]["SIZES"]["VALUE_ENUM_ID"],"NAME"=>$arOffer["PROPERTIES"]["SIZES"]["VALUE"],"SORT"=>$arOffer["PROPERTIES"]["SIZES"]["VALUE_SORT"],"PICT"=>"");//#ALICE
					//#ALICE
		...
		{
			$CATALOG_MEASURE_RATIO = $arItem['CATALOG_MEASURE_RATIO'];//#ALICE
			$arItem['CATALOG_MEASURE_RATIO'] = $arParams['SHOW_PRICE_COUNT'];//#ALICE
			CIBlockPriceTools::setRatioMinPrice($arItem, false);
			$arItem['CATALOG_MEASURE_RATIO'] = $CATALOG_MEASURE_RATIO;//#ALICE
	...
	//#ALICE
	$arSKUPropList["SIZES"]["VALUES"] = $arOfferSizes;
	$arSKUPropList["SIZES"]["VALUES_COUNT"] = count($arOfferSizes);
	//#ALICE
