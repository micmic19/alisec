...
$arCatalog = CIBlockSectionPropertyLink::GetArray($arParams["IBLOCK_ID"], $arCurSection['ID']);
$arSKU = CIBlockSectionPropertyLink::GetArray(3, $arCurSection['ID']);
$ar = array_merge($arCatalog, $arSKU);
If (count($ar) > 0) {
...
<?endif?>
<div class="<?=(($isFilter || $isSidebar) ? "col-md-9 col-sm-8 col-sm-push-4 col-md-push-3" : "col-xs-12")?>">
	<div class="row">
		<div class="col-xs-12">
	<?$APPLICATION->IncludeComponent(
...
$sort = array_key_exists("sort", $_REQUEST) && array_key_exists(ToLower($_REQUEST["sort"]), $arAvailableSort) ? $arAvailableSort[ToLower($_REQUEST["sort"])][0] : $arAvailableSort["price"][0];
...
<div class="sort hidden-xs">
	...
	<?
		...
		if (is_array($FILTER_VALUE["OFFERS"]["=PROPERTY_47"]) && is_array($FILTER_VALUE["OFFERS"]["=PROPERTY_48"]))
			$arParams["PAGE_ELEMENT_COUNT"] = 180;
		//#ALICE
?>
<!--#ALICE начало фрагмента Появляется только в мобильной версии-->
<?if (($isFilter || $isSidebar) & $isMobile):?>
	<?$APPLICATION->ShowViewContent("left_sidebar_filter")?>
	<?if ($isSidebar):?>
		<?$APPLICATION->IncludeComponent(
			"bitrix:main.include",
			"",
