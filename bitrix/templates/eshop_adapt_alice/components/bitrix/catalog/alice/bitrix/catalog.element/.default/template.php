...
$currencyList = '';
//#ALICE
function OfferPriceBySize($ID, $OFFERS){
	foreach ($OFFERS as $OFFER){
		if ($OFFER["PROPERTIES"]["SIZES"]["VALUE_ENUM_ID"] == $ID){
...
?>
	...
	<div id ="scissor" class ="scissor hidden-xs"></div>	
	<div class="bx_bigimages" id="<? echo $arItemIDs['BIG_IMG_CONT_ID']; ?>">
	<div class="bx_bigimages_imgcontainer">
		<div id="ruler-horizontal" class="ruler horizontal hidden-xs"></div>
		<div id="ruler-vertical" class="ruler vertical hidden-xs"></div>
...
?>
		<dt><? echo $arOneProp['NAME']; ?></dt><dd><?
			...
			?></dd><?
			if ($arOneProp['CODE'] == "THICKNESS" || $arOneProp['CODE'] == "PROTECTIVE_LAYER_HEIGHT") echo "<dd>мм.</dd>";//#ALICE			
		}
		unset($arOneProp);
?>
	</dl>
...
<?
			...
			foreach ($arProp['VALUES'] as $arOneValue){
				if ($arOneValue['ID'] == 0)
					continue;
				$Offerprices[$arOneValue['ID']] = round(OfferPriceBySize($arOneValue['ID'], $arResult['OFFERS'])["DISCOUNT_VALUE"]);
			}
			if (count(array_unique($Offerprices)) == 1)
			    unset($Offerprices);
			else
				foreach ($Offerprices as &$Offerprice)
					$Offerprice = number_format($Offerprice, 0)."р.";
			foreach ($arProp['VALUES'] as $arOneValue)
			{
...
<i title="<? echo $arOneValue['NAME']; ?>"></i><span class="cnt" title="<? echo $arOneValue['NAME']; ?>"><? echo $arOneValue['NAME']; ?></span><div><?=$Offerprices[$arOneValue['ID']]?></div></li>
<?
			}
?>
			</ul>
...
<?
			foreach ($arProp['VALUES'] as $arOneValue)
			{
				$arOneValue['NAME'] = htmlspecialcharsbx($arOneValue['NAME']);
?>
<li data-treevalue="<? echo $arProp['ID'].'_'.$arOneValue['ID'] ?>" data-onevalue="<? echo $arOneValue['ID']; ?>" style="padding-top: <? echo $strOneWidth; ?>; display: none;" >
...
$showSubscribeBtn = ($arParams['PRODUCT_SUBSCRIPTION'] == "Y");
	...
	<div class="item_buttons vam">
		<span class="item_buttons_counter_block">
			...
			<input id="<? echo $arItemIDs['QUANTITY']; ?>" type="text" class="tac transparent_input" value="<? echo (isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])
					? 1
					: $arParams['SHOW_PRICE_COUNT']// #ALICE $arResult['CATALOG_MEASURE_RATIO']
				); ?>">
...
<?
		...
		if ($showSubscribeBtn)
		{
			echo "<div style='display:none' id='subscribe_product'>";	
...
?>
		...
		<?$APPLICATION->ShowViewContent("catalog_store_amount")?>							
		<div class="bx_md">
<div class="item_info_section">
<?
if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
		...
		if ($arResult['SHOW_OFFERS_PROPS'])
		{
			if (!empty($arOneJS['DISPLAY_PROPERTIES']))
			{
				foreach ($arOneJS['DISPLAY_PROPERTIES'] as $arOneProp)
				{
					...
					if ($arOneProp['NAME'] == "Длина" || $arOneProp['NAME'] == "Ширина") $strProps .= "<dd>м.</dd>";//#ALICE				
					if ($arOneProp['NAME'] == "Ширина" && $arOneJS["MEASURE"] == "кв.м") $strProps .= "<dt style='color:red'>Нарезается по метражу в ширине ".$arOneProp['VALUE']." м.</dt>";
				}
			}
		}
		$arOneJS['DISPLAY_PROPERTIES'] = $strProps;
		$arOneJS['INIT_QUANTITY'] = $arParams['SHOW_PRICE_COUNT'];//#ALICE
	}
	if (isset($arOneJS))
		unset($arOneJS);
	$arJSParams = array(
		'CONFIG' => array(
			...
			'LABEL_SKU' => $arResult["LABEL_SKU"]//ALICE
		),
...
<?
	...
	$arJSParams = array(
		...
		'PRODUCT' => array(
			...
			'LENGTH' => $arResult['LENGTH'],//#ALICE
			'WIDTH' => $arResult['WIDTH'],//#ALICE
			'MEASURE' => $arResult['CATALOG_MEASURE_NAME']//#ALICE
		),
		'BASKET' => array(
...
if ($arParams['PRODUCT_SUBSCRIPTION'] == "Y")
	$arJSParams['SUBSCRIBE_URL_TEMPLATE'] = $templateFolder."/ajax.php";
