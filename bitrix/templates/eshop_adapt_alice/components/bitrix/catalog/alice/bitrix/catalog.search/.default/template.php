...
$arOffers[] = "1";
if ((!empty($arElements) && is_array($arElements)) || (!empty($arOffers) && is_array($arOffers)))
{
//#ALICE
	foreach($arElements as $k=>$v){
		$catsp_list = CCatalogProduct::GetList(array(), array("ID"=>$v));
		while ($arStore = $catsp_list->Fetch())
			if ($arStore["QUANTITY"]==0 && false)
				unset($arElements[$k]);
	}			
//#ALICE
	global $searchFilter;
