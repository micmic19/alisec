...
$arWaterMark = Array(
			array(
				"name" => "watermark",
				"position" => "bottomcenter", // Положение
				"type" => "image",
				"size" => "real",
				"alpha_level"=> 50,
				"file" => $_SERVER["DOCUMENT_ROOT"].'/include/logo.gif', // Путь к картинке
				"fill" => "exact",
			)
		);
//#ALICE
...
$arResult['LABEL_SKU'] = false;	
if ($arResult["PROPERTIES"]["MATERIAL"]["VALUE"] == "Шерсть"){
	$arResult['LABEL_VALUE'] = "Шерсть";
	$arResult['LABEL'] = true;
}
//ALICE Акция
foreach ($arResult["OFFERS"] as $keyOffer => $arItemOffer)
	if ($arItemOffer["PROPERTIES"]["IN_ACTION"]["VALUE"] == 1){
	$arResult['LABEL_VALUE'] = "Акция";
	$arResult['LABEL'] = true;
	$arResult['LABEL_SKU'] = true;	
}
//ALICE Акция		
//ALICE бугры
//foreach ($arResult["OFFERS"] as $keyOffer => $arItemOffer){
//	if ($arItemOffer["MIN_PRICE"]["DISCOUNT_DIFF_PERCENT"]=="20"){
//	$arResult['LABEL_VALUE'] = "на остаток в п. Бугры";
//	$arResult['LABEL'] = true;
//	$arResult['LABEL_SKU'] = true;
//	}	
//}
...
$arResult["MIN_PRICE"]["DISCOUNT_VALUE"] = round($arResult["MIN_PRICE"]["DISCOUNT_VALUE"]);
$arResult["MIN_PRICE"]['PRINT_DISCOUNT_VALUE'] = FormatCurrency($arResult["MIN_PRICE"]["DISCOUNT_VALUE"], $arResult["MIN_PRICE"]['CURRENCY']);
$arResult["MIN_PRICE"]['DISCOUNT_DIFF'] = $arResult["MIN_PRICE"]['VALUE'] - $arResult["MIN_PRICE"]["DISCOUNT_VALUE"];
$arResult["MIN_PRICE"]['PRINT_DISCOUNT_DIFF'] = FormatCurrency($arResult["MIN_PRICE"]['DISCOUNT_DIFF'], $arResult["MIN_PRICE"]['CURRENCY']);

//Group Name from section
$arFilterSection = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'GLOBAL_ACTIVE'=>'Y', 'ID'=>$arResult['IBLOCK_SECTION_ID']);
$db_list = CIBlockSection::GetList(false, $arFilterSection, false,array('NAME','IBLOCK_SECTION_ID', 'UF_GROUP_NAME'));
if($ar_result = $db_list->GetNext()){
	if (empty($ar_result['IBLOCK_SECTION_ID']) || !empty($ar_result['UF_GROUP_NAME']))
		if (empty($ar_result['UF_GROUP_NAME']))
			$arResult['NAME'] = $ar_result['NAME']." ".$arResult['NAME'];
		else
			$arResult['NAME'] = $ar_result['UF_GROUP_NAME']." ".$arResult['NAME'];
	else{	
		$arFilterSection['ID'] = $ar_result['IBLOCK_SECTION_ID'];
		$db_list = CIBlockSection::GetList(false, $arFilterSection, false, array('NAME', 'IBLOCK_SECTION_ID', 'UF_GROUP_NAME'));
		if($ar_result = $db_list->GetNext()){
			if (empty($ar_result['UF_GROUP_NAME']))
				$arResult['NAME'] = $ar_result['NAME']." ".$arResult['NAME'];
...
//#ALICE
foreach ($productSlider as &$productSliderItem){
	$arFileTmp = CFile::ResizeImageGet(
		$productSliderItem["ID"],
		array("width" => $productSliderItem["WIDTH"], "height" => $productSliderItem["HEIGHT"]),
		BX_RESIZE_IMAGE_EXACT,
		true,
		$arWaterMark
	);
	$productSliderItem["SRC"] = $arFileTmp["src"];	
}	
//#ALICE
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$CATALOG_MEASURE_RATIO = $arOffer['CATALOG_MEASURE_RATIO'];//#ALICE
		$arOffer['CATALOG_MEASURE_RATIO'] = $arParams['SHOW_PRICE_COUNT'];//#ALICE
		CIBlockPriceTools::setRatioMinPrice($arOffer, false);
		$arOffer['CATALOG_MEASURE_RATIO'] = $CATALOG_MEASURE_RATIO;//#ALICE		
	...
	{
		//#ALICE
		if (isset($arOffer['RATIO_PRICE'])){
			$arOffer["RATIO_PRICE"]["DISCOUNT_VALUE"] = round($arOffer["RATIO_PRICE"]["DISCOUNT_VALUE"]);
			$arOffer["RATIO_PRICE"]['PRINT_DISCOUNT_VALUE'] = FormatCurrency($arOffer["RATIO_PRICE"]["DISCOUNT_VALUE"], $arOffer["RATIO_PRICE"]['CURRENCY']);
			$arOffer["RATIO_PRICE"]['DISCOUNT_DIFF'] = $arOffer["RATIO_PRICE"]['VALUE'] - $arOffer["RATIO_PRICE"]["DISCOUNT_VALUE"];
			$arOffer["RATIO_PRICE"]['PRINT_DISCOUNT_DIFF'] = FormatCurrency($arOffer["RATIO_PRICE"]['DISCOUNT_DIFF'], $arOffer["RATIO_PRICE"]['CURRENCY']);
		}
		$arOffer["MIN_PRICE"]["DISCOUNT_VALUE"] = round($arOffer["MIN_PRICE"]["DISCOUNT_VALUE"]);
		$arOffer["MIN_PRICE"]['PRINT_DISCOUNT_VALUE'] = FormatCurrency($arOffer["MIN_PRICE"]["DISCOUNT_VALUE"], $arOffer["MIN_PRICE"]['CURRENCY']);
		$arOffer["MIN_PRICE"]['DISCOUNT_DIFF'] = $arOffer["MIN_PRICE"]['VALUE'] - $arOffer["MIN_PRICE"]["DISCOUNT_VALUE"];
		$arOffer["MIN_PRICE"]['PRINT_DISCOUNT_DIFF'] = FormatCurrency($arOffer["MIN_PRICE"]['DISCOUNT_DIFF'], $arOffer["MIN_PRICE"]['CURRENCY']);
		//#ALICE
		...
		$arFileTmp = CFile::ResizeImageGet(
			$firstPhoto["ID"],
			array("width" => $firstPhoto["WIDTH"], "height" => $firstPhoto["HEIGHT"]),
			BX_RESIZE_IMAGE_EXACT,
			true,
			$arWaterMark
		);
		$firstPhoto["SRC"] = $arFileTmp["src"];		
		//#ALICE
		$arOneRow = array(
			'ID' => $arOffer['ID'],
			'NAME' => $arOffer['~NAME'],
			...
			'LENGTH' => $arOffer["PROPERTIES"]["LENGTH"]["VALUE"],//#ALICE			
			'WIDTH' => $arOffer["PROPERTIES"]["WIDTH"]["VALUE"],//#ALICE
			'IN_ACTION' => (empty($arOffer["PROPERTIES"]["IN_ACTION"]["VALUE"])?false:true),//#ALICE плашка Акция
			'SUBSCRIBE_URL' => $arOffer["SUBSCRIBE_URL"],
		);
		//Бугры
		if ($arOffer["MIN_PRICE"]["DISCOUNT_DIFF_PERCENT"]=="20")
			$arOneRow["IN_ACTION"] = true;
		//Бугры
...
if ($arResult['MODULES']['catalog'] && $arResult['CATALOG'])
{
	...
	{	
		$CATALOG_MEASURE_RATIO = $arResult['CATALOG_MEASURE_RATIO'];//#ALICE
		$arResult['CATALOG_MEASURE_RATIO'] = $arParams['SHOW_PRICE_COUNT'];//#ALICE
		CIBlockPriceTools::setRatioMinPrice($arResult, false);
		$arResult['MIN_BASIS_PRICE'] = $arResult['MIN_PRICE'];
		$arResult['CATALOG_MEASURE_RATIO'] = $CATALOG_MEASURE_RATIO;//#ALICE
		$arResult['LENGTH'] = $arResult["PROPERTIES"]["LENGTH"]["VALUE"];//#ALICE			
		$arResult['WIDTH'] = $arResult["PROPERTIES"]["WIDTH"]["VALUE"];//#ALICE
