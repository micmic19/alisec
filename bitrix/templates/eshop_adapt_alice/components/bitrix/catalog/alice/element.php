...
if ($ElementID > 0)
{
	if($arParams["USE_STORE"] == "Y" && ModuleManager::isModuleInstalled("catalog"))
	{
		?><?$APPLICATION->IncludeComponent("bitrix:catalog.store.amount", ".default", array(
			...
			"SHOPS_IBLOCK_ID" => $arParams['SHOPS_IBLOCK_ID'],//#ALICE			
			"SELF_DELIVERY_PLACES_IBLOCK_ID" => $arParams['SELF_DELIVERY_PLACES_IBLOCK_ID'],//#ALICE
			"SELF_DELIVERY_PLACES_SHOP_CODE" => $arParams['SELF_DELIVERY_PLACES_SHOP_CODE']//#ALICE			
		),
		$component,
		array("HIDE_ICONS" => "Y")
	);?><?
